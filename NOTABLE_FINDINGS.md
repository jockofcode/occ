...
## OCC's Own pthread.h Had a Wrong `PTHREAD_MUTEX_INITIALIZER`

After encoding support was added, `test/ruby/test_string.rb` crashed inside `String#crypt` with:

```
pthread_mutex_lock: Invalid argument (EINVAL)
```
...
On macOS ARM64, `PTHREAD_MUTEX_INITIALIZER` expands to `{_PTHREAD_MUTEX_SIG_init, {0}}` where `_PTHREAD_MUTEX_SIG_init = 0x32AAABA7`. The `__sig` field is a magic cookie that `pthread_mutex_lock` validates before acquiring the lock.

The bug: OCC's bundled `lib/occ/include/pthread.h` takes precedence over the system headers, and it defined:

```c
#define PTHREAD_MUTEX_INITIALIZER  {0}
```

This zeroed `__sig`, making the mutex appear uninitialized. The lock call returned `EINVAL` and `crypt()` crashed immediately.

The root cause was not in the code generator — OCC correctly emits `.quad value` for nested struct initializers when the value is non-zero. The issue was entirely in the stub header.

**Fix:** Update `lib/occ/include/pthread.h` to use the real platform values:
...
**Lesson:** When OCC's own bundled headers shadow system headers, stub values must match the platform ABI exactly — zero-fill stubs are wrong for types that encode state in their initial bytes.

---

## Miniruby Encoding and Transcoder Support

Out-of-the-box miniruby only registers three encodings (ASCII-8BIT, US-ASCII, UTF-8) and the newline transcoder. Any call to `Encoding::UTF_16BE` or `"str".encode("EUC-JP")` would raise `NameError` / `ConverterNotFoundError`.

**Root cause (encodings):** CRuby's `miniinit.c` defines a minimal `Init_enc()` that calls `rb_encdb_declare` for only three encodings. The extra encoding `.c` files (like `enc/euc_jp.c`) exist but their `Init_XXX()` functions are never called.

**Root cause (transcoders):** Transcoders are normally loaded as dynamic extensions. Miniruby's `Init_ext()` is an empty stub. The `transcode.c` file only calls `Init_newline()` (the built-in newline normalizer) directly.

**Fix:** The OCC phase-11 harness (`spec/phase11/ruby_spec.rb`):
